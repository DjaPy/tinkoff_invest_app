...
async def list_positions(
    ...
    if strategy_id:
        query_filters.append(PortfolioPositionDocument.strategy_id == strategy_id)

    if instrument:
        query_filters.append(PortfolioPositionDocument.instrument == instrument)

    positions = await PortfolioPositionDocument.find_all().to_list()
    if query_filters:
        positions = await PortfolioPositionDocument.find(*query_filters).to_list()
...
@positions_router.get(
    '/{position_id}',
    response_model=PortfolioPositionDocument,
    summary='Get position details',
    description='Retrieve detailed information about a specific portfolio position',
)
async def get_position(position_id: UUID) -> PortfolioPositionDocument:
    """
    ...
    Raises:
        ...
        HTTPException 500: Internal server error
    """
    position = await PortfolioPositionDocument.find_one(PortfolioPositionDocument.position_id == position_id)
