...
async def trading_api_exception_handler(request: Request, exc: TradingAPIException) -> JSONResponse:
...
async def validation_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    invalid_params = [{'name': '.'.join(str(loc) for loc in err['loc']), 'reason': err['msg']} for err in exc.errors()]
    ...
    return JSONResponse(status_code=HTTPStatus.UNPROCESSABLE_ENTITY, content=problem.model_dump(exclude_none=True))
