...
class TradeOrder(Document):
    ...
    price: Decimal | None = Field(None, gt=0, description="Limit price (None for market)")
    ...
    filled_price: Decimal | None = Field(None, gt=0, description="Actual execution price")
    ...
    def update_status(
        ...
        if filled_quantity is not None:
            if filled_quantity > self.quantity:
                raise ValueError(
                    f"Filled quantity {filled_quantity} exceeds order quantity {self.quantity}"
                )
